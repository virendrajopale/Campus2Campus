...
const handleSendVerificationOTP = async (req, res) => {
    ...
    res.status(200).json({msg:"OTP Has Been Sent To Provided Email Address"});
...
const handleLogin = async (req, res) => {
    try {
        ...
        if (role == "Student") {
            ...
            bcrypt.compare(password, stu.password, function (err, result) {
                if (result) {
                    ...
                    return res.json({ tocken: authTocken, user: stu.name, role: "Student", msg: "Login Successful" })
                    //  res.json({ user:stu})
                } else {
        ...
        else if (role == "Admin" || role=='TPO_Dept_Admin') {
            ...
            bcrypt.compare(password, TPO.password, function (err, result) {
                if (result) {
                    ...
                    //  res.json({ user:TPO})
                    return res.json({ tocken: authTocken, user: TPO.name, role: TPO.role, msg: "Login successful" });
                }
                else {
                    return res.status(401).send({ msg: "Invalid credentials" });
                }
...
const handleRegistrationReq = async (req, res) => {
    ...
    try {
        if (req.user.role === "TPO_Dept_Admin") {
            console.log("TPO_Dept_Admin");
            ...
            const requests = await PendingReq.find({ dept: req.user.dept }).populate({ path: 'OriginalStu', select: '-password -CompanyName -applicationHistory -placed' }).select("-password");
            return res.send({ requests, msg: "Registration requests fetched successfull" });
        }
