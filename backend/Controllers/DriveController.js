...
const handleApplyToDrive = async (req, res) => {
    ...
    if( drive.EligibleDepartMents.length===0 && drive.EligibleYears.length===0){
        return res.status(403).json({msg:"Sorry For Inconvenience,This is Server Problem We will Get Back To You"})
    }
    if ((!drive.EligibleDepartMents.includes(stu.dept) || !drive.EligibleYears.includes(stu.year))  ) {
        return res.status(401).send({ msg: "Not Eligible to apply to this Drive" });
    }
    let isapplied = stu.applicationHistory.find((appl) => appl.DriveId.toString() === req.params.id);
    if (isapplied) {
        return res.status(406).send({ msg: "You have already applied for the drive" });
    }
    ...
    res.status(201).send({ stu , msg: "Applied Successfully" });
}
...
const handleDriveStudent = async (req, res) => {
    try {
        ...
        return res.json({ students, msg: "Stundets fetched Successfully" });
    } catch (error) {
        return res.status(500).json({ messge: error.message, msg: "Internal server Error" });
    }
}
